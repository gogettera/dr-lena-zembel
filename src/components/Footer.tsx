
import React from 'react';
import Logo from './Logo';
import { Phone, MapPin, Clock } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { useLanguage } from '@/contexts/LanguageContext';
import { Separator } from '@/components/ui/separator';
import { Link } from 'react-router-dom';
import { createLocalizedPath } from '@/utils/languageRoutes';
import { treatmentTypes, getTreatmentNameKey } from '@/data/treatmentTypes';

const Footer = () => {
  const { t, language } = useLanguage();
  const isRTL = language === 'he' || language === 'ar';
  
  const mainLinks = [
    { key: 'home', href: '/' },
    { key: 'about', href: '/#about' },
    { key: 'contact', href: '/#contact' },
  ];

  // Get all treatment types for the footer
  const treatmentLinks = Object.keys(treatmentTypes).map(treatmentType => ({
    key: getTreatmentNameKey(treatmentType),
    href: `/treatments/${treatmentType}`,
    labelKey: getTreatmentNameKey(treatmentType)
  }));

  const legalLinks = [
    { key: 'privacy', href: '/privacy', labelKey: 'privacy' },
    { key: 'terms', href: '/terms', labelKey: 'terms' }
  ];

  const socialLinks = [
    {
      name: 'Facebook',
      url: 'https://www.facebook.com/drzembel',
      icon: (
        <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          <path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd" />
        </svg>
      ),
    },
    {
      name: 'Instagram',
      url: 'https://www.instagram.com/lena.zembel/',
      icon: (
        <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          <path fillRule="evenodd" d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z" clipRule="evenodd" />
        </svg>
      ),
    },
    {
      name: 'LinkedIn',
      url: 'https://il.linkedin.com/in/lena-zembel',
      icon: (
        <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
        </svg>
      ),
    },
  ];

  return (
    <footer id="contact" className="bg-dental-navy text-white py-16 relative overflow-hidden" role="contentinfo">
      <div className="absolute inset-0 bg-[url('/lovable-uploads/11fa7c9b-39fc-4d60-b09b-13f0578ebffe.png')] bg-cover bg-center opacity-5 mix-blend-overlay"></div>
      
      <div className="container mx-auto px-4 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-16">
          {/* Contact Info Column */}
          <div className="text-center md:text-right">
            <div className="mb-6">
              <Logo />
            </div>
            <div className="space-y-4 mt-6">
              <div className="flex items-center justify-center md:justify-end gap-3">
                <Phone className="h-5 w-5 text-dental-orange flex-shrink-0" />
                <span className="text-white">03-566-6915</span>
              </div>
              <div className="flex items-center justify-center md:justify-end gap-3">
                <MapPin className="h-5 w-5 text-dental-orange flex-shrink-0" />
                <span className="text-white">{t('clinicInfo.address')}</span>
              </div>
            </div>
            <div className="flex justify-center md:justify-end gap-4 mt-6">
              {socialLinks.map((social) => (
                <a
                  key={social.name}
                  href={social.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-white/10 hover:bg-white/20 p-2 rounded-full transition-colors"
                >
                  <span className="sr-only">{social.name}</span>
                  {social.icon}
                </a>
              ))}
            </div>
          </div>

          {/* Hours & Location Column */}
          <div className="text-center md:text-right">
            <h3 className="text-xl font-bold text-white mb-6 inline-block pb-2 border-b-2 border-dental-orange">
              {t('openingHours')}
            </h3>
            <div className="space-y-3">
              <div className="flex items-center justify-center md:justify-end gap-3">
                <Clock className="h-5 w-5 text-dental-orange flex-shrink-0" />
                <div>
                  <p className="text-white/80">{t('sundayToFriday')}</p>
                  <p className="text-white">8:30-14:00</p>
                </div>
              </div>
            </div>
          </div>

          {/* Navigation Column */}
          <div className="text-center md:text-right">
            <h3 className="text-xl font-bold text-white mb-6 inline-block pb-2 border-b-2 border-dental-orange">
              {t('sitemap')}
            </h3>
            <div className="grid grid-cols-2 gap-8">
              {/* Main Links */}
              <div>
                <h4 className="text-lg font-semibold mb-4 text-dental-orange">{t('navigation')}</h4>
                <ul className={`space-y-2 ${isRTL ? 'text-right' : 'text-left'}`}>
                  {mainLinks.map((link) => (
                    <li key={link.key}>
                      <Link
                        to={createLocalizedPath(language, link.href)}
                        className="text-white/80 hover:text-white transition-colors"
                      >
                        {t(link.key)}
                      </Link>
                    </li>
                  ))}
                </ul>
              </div>

              {/* Treatments Links */}
              <div>
                <h4 className="text-lg font-semibold mb-4 text-dental-orange">{t('treatments')}</h4>
                <ul className={`space-y-2 ${isRTL ? 'text-right' : 'text-left'}`}>
                  {treatmentLinks.map((link) => (
                    <li key={link.key}>
                      <Link
                        to={createLocalizedPath(language, link.href)}
                        className="text-white/80 hover:text-white transition-colors"
                      >
                        {t(link.labelKey)}
                      </Link>
                    </li>
                  ))}
                </ul>
              </div>
            </div>
          </div>
        </div>

        {/* Footer Bottom */}
        <div className="mt-16 pt-6 border-t border-white/10">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-sm text-white/60">© {new Date().getFullYear()} {t('copyright')}.</p>
            <div className="flex gap-4">
              {legalLinks.map((link) => (
                <Link
                  key={link.key}
                  to={createLocalizedPath(language, link.href)}
                  className="text-sm text-white/60 hover:text-white transition-colors"
                >
                  {t(link.labelKey)}
                </Link>
              ))}
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
